/**
 * ==DOMCached local storage library, version 0.1c==
 ...
 * 
 * NB! DOMCached requires Prototype (prototypejs.org) to convert from/to JSON strings and also for <IE8 userData.
 * This functionality can also be achieved using other libraries (jQuery, etc.), minor editing might be needed.
 * 
**/
...
DOMCached = {
	...
	init: function(){
		if("localStorage" in window){
			...
			if("addBehavior" in (new Element('div'))){
		...
		if("dom_storage" in this.storage_service && this.storage_service.dom_storage){
			try{
				this.storage = String(this.storage_service.dom_storage).evalJSON();
	...
	save:function(){
		if(this.storage_service){
			try{
				this.storage_service.dom_storage = Object.toJSON(this.storage);
			}catch(E){/* probably cache is full, nothing is saved this way*/}
			// If userData is used as the storage engine, additional 
			if($('elm_domcached')){
				try{
					$('elm_domcached').setAttribute("domcached",this.storage_service.dom_storage)
	...
	'delete': function(key, namespace){
		...
		if(this.storage[namespace] && this.storage[namespace][key]){
			...
			for(var i in delete this.storage[namespace]){
